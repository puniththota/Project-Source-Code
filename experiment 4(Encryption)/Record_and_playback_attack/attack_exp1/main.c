...
void loop() {
	...
	if (radio_rxed()) {
		...
		unsigned char valid;
		int rxLen = radio_recv(&buffer[0],128,&dst,&src,&valid,&energy,&lqi);   //recording the data packet being transmitted and storing it
	...
	if (button_pressed()) {
		...
		leds_on(LED_GREEN);
		radio_send(buffer,4,0x0002);   //playing back the recorded data packet
